        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (!excludeDirs.includes(entry.name))
                    walkDir(fullPath, extensions, excludeDirs).forEach(r => results.push(r));
            }
...
function scanSQLInjection(files, projectRoot) {
    ...
    for (const filePath of files) {
        try {
            ...
            if (rel.includes('security-watchdog'))
                continue;
            content.split(/\r?\n/).forEach((line, idx) => {
...
async function runScan() {
    ...
    if (!config) {
        console.error(`${C.red}❌ vibe-security.config.js bulunamadı!${C.reset}`);
        process.exit(1);
    }
    if (!config.enabled) {
        console.log(`${C.dim}🛡️  Devre dışı.${C.reset}`);
        return;
    }
    console.log('');
    console.log(DLINE);
    ...
    for (const dir of scanDirs) {
        ...
        if (fs.existsSync(dirPath))
            allFiles.push(...walkDir(dirPath, extensions, excludeDirs));
    }
    // ─── 1. Secret Scanner ───
    ...
    if (config.rlsScanner?.enabled) {
        ...
        if (filteredTables.size > 0) {
            ...
            for (const [tableName] of filteredTables) {
                if (whitelisted.includes(tableName))
                    continue;
    ...
    const parts = [];
    if (criticalCount > 0)
        parts.push(`${C.red}${C.bold}${criticalCount} Kritik${C.reset}`);
    if (warningCount > 0)
        parts.push(`${C.yellow}${warningCount} Uyarı${C.reset}`);
    if (infoCount > 0)
        parts.push(`${C.blue}${infoCount} Bilgi${C.reset}`);
    console.log(DLINE);
    ...
    for (const issue of sorted) {
        ...
        if (issue.file)
            console.log(`  ${C.dim}📄 ${issue.file}${issue.line ? `:${issue.line}` : ''}${C.reset}`);
        console.log(`  ${C.dim}${issue.message}${C.reset}`);
        if (issue.context)
            console.log(`  ${C.dim}📝 ${issue.context.substring(0, 120)}${C.reset}`);
        console.log('');
    }
    // ─── vibe-summary.txt ───
...
//# sourceMappingURL=security-watchdog.js.map